...
def draw2(img, pred):
    ...
    if len(pred):
        for detect in pred:
            ...
            labels =  ['backpack','umbrella','handbag','tie','suitcase','bottle','laptop','mouse','remote','cell phone','book']
    ...
    if len(pred):
        for detect in pred:
            ...
            if(mark==1):
                ...
                img_ = cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 2)
                text = labels[int(cls)]
                cv2.putText(img, "items", (int((x1+x2)/2),int((y1+y2)/2)), cv2.FONT_HERSHEY_SIMPLEX, 4, (0, 0, 255),4)
    return img_
...
if __name__ == '__main__':
    ...
    pred1 = main('F:\\72person.ckpt', img)
    pred1 = nms(pred1, 0.1, 0.1)
    ret_img1 = draw2(img,pred1)
    ret_img1 = ret_img1[:, :, ::-1]
    plt.imshow(ret_img1)
    plt.savefig("F:\\T1.png")
    plt.show()